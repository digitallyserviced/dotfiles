...
local color_list = { "black", "red", "green", "yellow", "blue", "magenta", "cyan", "white" }
--, "orange", "pink"
--#region Types ----------------------------------------------------------------
---@class Color
...
local bitopt = require("lib.bit").bit
...
-- function RO(num, numDecimalPlaces)
--   numDecimalPlaces = numDecimalPlaces or 2
--   local mult = 10^(numDecimalPlaces or 0)
--   return math.floor(num * mult + 0.5) / mult
-- end
function RO(exact)
  return tonumber(string.format("%.6f", exact + 0.001))
end
function Color.init(r, g, b, a)
  local self = setmetatable({}, Color)
  self.red = RO(util.clamp(r, 0, 1))
  self.green = RO(util.clamp(g, 0, 1))
  self.blue = RO(util.clamp(b, 0, 1))
  self.alpha = RO(util.clamp(a or 1, 0, 1))
  return self
end
...
function Color:to_hex(with_alpha)
  -- print(string.format("%s, %s, %s, %s, %s, %s", self.blue, self.green, self.red, self.blue * 0xff, RO(self.green)*0xff, self.red*0xff))
  -- local n = (
  ...
  local buffer="#"
  for i, v in ipairs({self.red, self.green, self.blue}) do
