...
vec4 junk( vec4 gl_FragColor, vec2 gl_FragCoord ){
  float a = gl_FragColor.a;
    ...
    float t = time*20.;
    float rows = floor(50.0);
    vec2 ipos = floor(st*rows);
    vec2 fpos = fract(st*rows);

    ipos.y += mod(floor(t*random(ipos.x+1.)),max(resolution.x,resolution.y));

    float pct = 1.076;
    pct *= randomChar(ipos,fpos);
    pct *= random(ipos);

    color += pct*pct; // /step(0.1, 1.0-pct);

    gl_FragColor = vec4( color, step(0.01, pct*0.8));
    return gl_FragColor;
}
void main() {
  ...
  color=mix(color, junk(color, o_position), 0.1);
