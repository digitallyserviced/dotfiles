...
__bp_interactive_mode() {
    __wezterm_quick_var interactive 1
...
__bp_precmd_invoke_cmd() {
    ...
    # __wezterm_mark_time cmdend 
    __bp_last_ret_value="$?" BP_PIPESTATUS=("${PIPESTATUS[@]}")
    # __wezterm_mark_time time_precmd_invoke
    ...
    for precmd_function in "${precmd_functions[@]}"; do
        # __wezterm_mark_time time_precmd_${precmd_function}_invoke
...
__bp_preexec_invoke_exec() {
    # __wezterm_mark_time time_preexec_invoke
    ...
    for preexec_function in "${preexec_functions[@]:-}"; do
        # __wezterm_mark_time time_preexec_${preexec_function}_invoke
...
## Do OSC 1337 to pass user data too the term
# __wezterm_user_var <outvar> <name> <data>
# for example with iterm setting a profile for the term
# local user_var=""
# 727 678 1429
# __wezterm_user_var 'user_var' 'SetProfile' 'ProfileName'
# print ${user_var}
__wezterm_last_cmd_info(){
  xc=$1
  typeset -A vars=()
  vars[job]="%j"
  vars[exit]="${xc}"
  ...
  for k v in ${(%kv)vars}; do 
    local out=""
    __wezterm_user_var out "${k}" ${v} 
    uservars+=("${out}")
    __wezterm_tmux_maybe_wrap ${out}
  done
  last_cmd_info=(${(qq)uservars[@]})
}
__wezterm_push_vars(){
  return
  local -a d=(${1})
  for v in ${(z)d[@]}; do 
    __wezterm_tmux_maybe_wrap "${v}"
  done
}
__wezterm_user_var(){
  local outvar=${1}
  print -v ${outvar} -f '\033]1337;SetUserVar=%s=%s\a' "$k" "$(echo ${(qq)v} | base64)"
}
__wezterm_tmux_maybe_wrap(){
  local seq=${1}
  # trap '__wezterm_close_escseq_wrap; trap - EXIT' EXIT
  if [[ -n "${TMUX}" ]] ; then
    printf '\033Ptmux;\033%s\033\\' "$seq"
    # printf "\033tmux;\033\033]${seq}\007\033\\"
  else
    printf '\033]%s\007' "${seq}"
  fi
}
...
__wezterm_quick_var(){
  local k=$1 v=$2
  local outv=""
  __wezterm_user_var outv "${name}" "${value}"
  __wezterm_tmux_maybe_wrap "${outv}"
}
__wezterm_mark_time(){
  local annotation=$1
  local -F 4 starttime=`date +%s.%N`
  __wezterm_quick_var ${annotation} "${starttime}"
}

# functions -T __wezterm_tmux_maybe_wrap
# functions -T __wezterm_last_cmd_info
# functions -T __wezterm_push_vars
# functions -T __wezterm_user_var
...
__wezterm_semantic_precmd() {
  ...
  if [[ "$__wezterm_semantic_precmd_executing" != "" ]] ; then
    ...
    __wezterm_last_cmd_info $ret
    # __wezterm_push_vars ${()last_cmd_info[@]}
...
function __wezterm_semantic_preexec() {
  ...
  # local -F 4 starttime=`date +%s.%N`
  # __wezterm_quick_var cmdstart "${starttime}"
  # __wezterm_mark_time time_preexec 
  # _stamp_preexec
...
if [[ -z "${WEZTERM_SHELL_SKIP_CWD}" ]] ; then
  ...
  setopt no_prompt_{bang,subst} prompt_{percent,sp}
